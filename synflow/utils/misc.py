...
def get_experiment_name(model_name: str, version: str, version_time: datetime.datetime | None = None):
    # Replace illegal filename characters with _
    illegal_chars = re.compile(r"[\\/:\"*?<>|]+")
    model_name = illegal_chars.sub("_", model_name)
    version = version
    time_prefix = version_time.strftime("%y%m%d%H%M") if version_time is not None else get_experiment_version()
