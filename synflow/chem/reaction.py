...
class Reaction(Drawable):
    ...
    def __call__(self, reactants: Sequence[Molecule]) -> list[Molecule]:
        ...
        for i, reactant in enumerate(reactants):
            for atom in reactant._rdmol.GetAtoms():
                atom.SetAtomMapNum(index)
                index += 1
        for i, reactant in enumerate(reactants):
            print(f"Reactant {i+1}:", Chem.MolToSmiles(reactant._rdmol))
            print("Atom Mapping:", [(atom.GetSymbol(), atom.GetAtomMapNum()) for atom in reactant._rdmol.GetAtoms()])

        products = self._reaction.RunReactants([m._rdmol for m in reactants])

        for i, product_set in enumerate(products):  
            print(f"Reaction Result {i+1}:")
            for product in product_set:
                print("Product SMILES:", Chem.MolToSmiles(product))
                print("Atom Mapping:", [(atom.GetSymbol(), atom.GetAtomMapNum()) for atom in product.GetAtoms()])
